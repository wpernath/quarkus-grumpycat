...
CONTEXT_DIR=.
IMAGE_NAME=quay.io/wpernath/grumpycat-melonjs
IMAGE_USER=wpernath
IMAGE_PASSWORD=
...
kind: PipelineRun
metadata:
  name: client-stage-pipeline-run-$(date "+%Y%m%d-%H%M%S")
spec:
  params:
    - name: release-name
      value: $GIT_REVISION
...
kind: PipelineRun
metadata:
  name: client-dev-pipeline-run-$(date "+%Y%m%d-%H%M%S")
spec:
  params:
    - name: repo-password
